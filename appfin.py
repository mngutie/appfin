...
# Faturas de cartão — só alerta se não paga
for fat in faturas:
    cartao_obj = next((c for c in cartoes if c["id"] == fat.get("cartao_id")), None)
    if cartao_obj:
        for p in range(fat.get("parcelas", 1)):
            ref_id = f"fat_{fat['id']}_{p}"
            if esta_pago(ref_id):
                continue
            mes_fat = mes_from_label(fat["mes_inicio"])
            mes_p   = mes_fat + relativedelta(months=p) if p > 0 else mes_fat
            if isinstance(mes_p, str):
                mes_p = mes_from_label(mes_p)
            dia     = min(cartao_obj["dia_venc"], calendar.monthrange(mes_p.year, mes_p.month)[1])
            dt_fat  = date(mes_p.year, mes_p.month, dia)
            diff    = (dt_fat - hoje).days
            if 0 <= diff <= 5:
                alertas.append(("warn", f"💳 Fatura {cartao_obj['nome']} — {fat['descricao']} vence em {diff} dia(s)"))
            elif -3 <= diff < 0:
                alertas.append(("danger", f"🔴 Fatura {cartao_obj['nome']} — {fat['descricao']} venceu há {-diff} dia(s)"))
    ...
    # ════════════════════════════════════════════════════════════════════════════
        ...
        if cartoes:
            for cartao in cartoes:
                fats_cartao = [f for f in faturas if f.get("cartao_id") == cartao["id"]]
                total_cartao = sum(f["valor"] for f in fats_cartao)
                uso_limite   = (total_cartao / cartao["limite"] * 100) if cartao["limite"] > 0 else 0
                cor_uso      = "#38a169" if uso_limite<=70 else "#d69e2e" if uso_limite<=90 else "#e53e3e"

                st.markdown(f"""
                <div style="background:#0d0f17;border:1px solid #1c1f2e;border-left:3px solid #3182ce;
                     border-radius:2px;padding:20px 24px;margin:10px 0">
                  <div style="display:flex;justify-content:space-between;align-items:flex-start">
                    <div>
                      <div style="font-size:16px;font-weight:700;color:#e2e8f0">{cartao['nome']}</div>
                      <div style="font-size:11px;color:#4a5568;margin-top:2px">
                        Fecha dia {cartao['dia_fech']} · Vence dia {cartao['dia_venc']}
                      </div>
                    </div>
                    <div style="text-align:right">
                      <div style="font-size:11px;color:#4a5568;text-transform:uppercase;letter-spacing:1px">Limite</div>
                      <div style="font-family:IBM Plex Mono,monospace;font-size:18px;font-weight:600;color:#e2e8f0">R$ {cartao['limite']:,.2f}</div>
                    </div>
                  </div>
                  <div style="margin-top:14px;display:flex;gap:24px">
                    <div><div style="font-size:10px;color:#4a5568;letter-spacing:1px;text-transform:uppercase">Total utilizado</div>
                         <div style="font-family:IBM Plex Mono,monospace;font-size:16px;color:#e53e3e;font-weight:600">R$ {total_cartao:,.2f}</div></div>
                    <div><div style="font-size:10px;color:#4a5568;letter-spacing:1px;text-transform:uppercase">Disponível</div>
                         <div style="font-family:IBM Plex Mono,monospace;font-size:16px;color:#38a169;font-weight:600">R$ {max(cartao['limite']-total_cartao,0):,.2f}</div></div>
                    <div><div style="font-size:10px;color:#4a5568;letter-spacing:1px;text-transform:uppercase">Uso</div>
                         <div style="font-family:IBM Plex Mono,monospace;font-size:16px;font-weight:600;color:{cor_uso}">{uso_limite:.1f}%</div></div>
                    <div><div style="font-size:10px;color:#4a5568;letter-spacing:1px;text-transform:uppercase">Lançamentos</div>
                         <div style="font-family:IBM Plex Mono,monospace;font-size:16px;color:#c9d1d9">{len(fats_cartao)}</div></div>
                  </div>
                  <div class="prog-bg" style="margin-top:12px">
                    <div class="prog-fill" style="width:{min(uso_limite,100):.0f}%;background:{cor_uso}"></div>
                  </div>
                </div>
                """, unsafe_allow_html=True)

                if fats_cartao:
                    with st.expander(f"Ver lançamentos — {cartao['nome']}"):
                        html = "<table><tr><th>Descrição</th><th>Categoria</th><th style='text-align:right'>Valor Total</th><th style='text-align:right'>Parcelas</th><th>Período</th></tr>"
                        for f in sorted(fats_cartao, key=lambda x: x["mes_inicio"]):
                            fim_f = mes_label(mes_from_label(f["mes_inicio"]) + relativedelta(months=f["parcelas"]-1))
                            html += (f"<tr><td>{f['descricao']}</td><td>{f.get('categoria','')}</td>"
                                     f"<td style='text-align:right'>{fmt_mon(f['valor'])}</td>"
                                     f"<td style='text-align:right;color:#4a5568'>{f['parcelas']}x R$ {f['valor']/f['parcelas']:,.2f}</td>"
                                     f"<td>{f['mes_inicio']} → {fim_f}</td></tr>")
                        html += "</table>"
                        st.markdown(html, unsafe_allow_html=True)
        else:
            st.info("Nenhum cartão cadastrado.")
