...
def cached_load():
    ...
    return transacoes, contas_fixas, cartoes, faturas, metas

def reload():
    st.cache_data.clear()
    st.rerun()

transacoes, contas_fixas, cartoes, faturas, metas = cached_load()
...
st.markdown(f"<p style='color:#6666a0;margin-top:0'>{dia_semana_pt(datetime.now())}, {datetime.now().strftime('%d/%m/%Y')}</p>", unsafe_allow_html=True)
...
with tab1:
    ...
    def mcard(col, label, val, cls, extra=""):
        col.markdown(f'<div class="card"><div class="metric-label">{label}</div>'
                     f'<div class="metric-value {cls}">R$ {val:,.2f}</div>{extra}</div>',
                     unsafe_allow_html=True)
...
with tab5:
    ...
    if not df_cal.empty:
            ...
            for dia in sem:
                if dia == 0:
                    ...
                    for _, row in contas_dia.iterrows():
                        cor = "#60a5fa" if row.get("origem") == "cartao" else "#a78bfa" if row.get("origem") == "fixa" else "#f87171"
                        cell += (f'<div style="background:{cor}22;color:{cor};font-size:10px;'
                                 f'border-radius:6px;padding:2px 5px;margin-top:2px;overflow:hidden;'
                                 f'white-space:nowrap;text-overflow:ellipsis">'
                                 f'{row["descricao"][:16]} R${row["valor"]:,.0f}</div>')
        ...
        df_cal_s["Data"]  = df_cal_s["data"].dt.strftime("%d/%m/%Y")
