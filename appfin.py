...
for cf in contas_fixas:
    ...
    for i in range(cf["meses"]):
        ...
        diff  = (dt - hoje).days
        if 0 <= diff <= 5:
            alertas.append(("warn", f"⚠️ {cf['descricao']} vence in {diff} dia(s) — R$ {cf['valor']:,.2f}"))
        elif -3 <= diff < 0:
...
# ABAS
# ════════════════════════════════════════════════════════════════════════════
tab1,tab2,tab3,tab4,tab5,tab6,tab_rel = st.tabs([
    "🏠 Resumo","📝 Lançamentos","📌 Contas Fixas","💳 Cartões","📅 Calendário","📈 Projeção", "📊 Relatório Detalhado"])
                ...
                for dia in sem:
                    if dia == 0:
                        ...
                        for _, row in contas_dia.iterrows():
                            ...
                            cor = "#38a169" if pago else ("#3182ce" if row.get("origem") == "cartao" else "#805ad5" if row.get("origem") == "fixa" else "#e53e3e")
                            cell += f'<div style="background:{cor}22;color:{cor};font-size:9px;padding:2px;margin-top:2px;border-radius:2px">{row["descricao"][:12]}</div>'
                        html += f'<td style="height:90px;vertical-align:top;padding:6px;background:{bg};border:{brd}">{cell}</td>'
                html += '</tr>'
            html += '</table>'
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
with tab6:
    st.markdown("### 📈 Projeção Financeira (Próximos 6 meses)")
    if not df_all.empty:
        proj_meses = gerar_meses(7)[3:] # Do atual em diante
        proj_data = []
        for m in proj_meses:
            df_m = df_all[df_all["mes"] == m]
            r = df_m[df_m["tipo"] == "Receita"]["valor"].sum()
            d = df_m[df_m["tipo"] == "Despesa"]["valor"].sum()
            proj_data.append({"Mês": m, "Receitas": r, "Despesas": d, "Saldo": r-d})
        
        df_p = pd.DataFrame(proj_data)
        fig_p = go.Figure()
        fig_p.add_trace(go.Scatter(x=df_p["Mês"], y=df_p["Receitas"], name="Receitas", line=dict(color="#4ade80", width=3)))
        fig_p.add_trace(go.Scatter(x=df_p["Mês"], y=df_p["Despesas"], name="Despesas", line=dict(color="#f87171", width=3)))
        fig_p.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#e8e8f0")
        st.plotly_chart(fig_p, use_container_width=True)

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# ABA RELATÓRIO — DETALHADO E COMPLETO
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
with tab_rel:
    st.markdown("### 📊 Relatório Executivo de Performance")
    
    # Seleção de Mês para o Relatório
    meses_rel = sorted(df_all["mes"].unique().tolist(), reverse=True) if not df_all.empty else [mes_label(date.today())]
    m_rel = st.selectbox("Selecionar Período para Análise", meses_rel, key="m_rel")
    ...
    if not df_rel.empty:
        ...
        saldo_r = total_r - total_d
        
        # 1. Indicadores de Eficiência
        st.markdown("#### 🚀 Indicadores de Saúde Financeira")
        i1, i2, i3, i4 = st.columns(4)
        
        taxa_poupanca = (saldo_r / total_r * 100) if total_r > 0 else 0
        i1.metric("Taxa de Poupança", f"{taxa_poupanca:.1f}%", help="Porcentagem da receita que sobrou")
        
        fixas_total = df_rel[df_rel["origem"] == "fixa"]["valor"].sum()
        i2.metric("Custo Fixo", f"R$ {fixas_total:,.2f}", f"{(fixas_total/total_r*100 if total_r>0 else 0):.1f}% Rec.")
        
        variaveis_total = df_rel[df_rel["origem"] == "manual"]["valor"].sum()
        i3.metric("Gastos Variáveis", f"R$ {variaveis_total:,.2f}", delta_color="inverse")
        
        cartao_total = df_rel[df_rel["origem"] == "cartao"]["valor"].sum()
        i4.metric("Comprometimento Cartão", f"R$ {cartao_total:,.2f}", f"{(cartao_total/total_d*100 if total_d>0 else 0):.1f}% Desp.")

        st.markdown("---")
        
        # 2. Análise de Pareto (Maiores Gastos)
        c_left, c_right = st.columns([1, 1.5])
        
        with c_left:
            st.markdown("#### 📉 Ranking de Despesas")
            rank = df_rel[df_rel["tipo"] == "Despesa"].groupby("categoria")["valor"].sum().sort_values(ascending=False).reset_index()
            rank["%"] = (rank["valor"] / total_d * 100).map("{:.1f}%".format)
            st.table(rank)
            
        with c_right:
            st.markdown("#### 🕒 Fluxo de Caixa Diário")
            df_rel['dia'] = df_rel['data'].dt.day
            fluxo = df_rel.groupby(['dia', 'tipo'])['valor'].sum().unstack(fill_value=0).reset_index()
            # Garante que as colunas existam
            if 'Receita' not in fluxo: fluxo['Receita'] = 0
            if 'Despesa' not in fluxo: fluxo['Despesa'] = 0
            
            fluxo['Saldo Acumulado'] = (fluxo['Receita'] - fluxo['Despesa']).cumsum()
            
            fig_fluxo = go.Figure()
            fig_fluxo.add_trace(go.Scatter(x=fluxo['dia'], y=fluxo['Saldo Acumulado'], 
                                         fill='tozeroy', name="Saldo Acum.", line=dict(color="#3182ce")))
            fig_fluxo.update_layout(height=300, margin=dict(t=0, b=0, l=0, r=0), 
                                  paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                                  xaxis=dict(title="Dia do Mês", showgrid=False),
                                  yaxis=dict(title="R$ Acumulado", gridcolor="#1c1f2e"))
            st.plotly_chart(fig_fluxo, use_container_width=True)

        st.markdown("---")
        
        # 3. Detalhamento Técnico
        st.markdown("#### 🔍 Conciliação Detalhada")
        df_view = df_rel.sort_values("data")[["data", "tipo", "descricao", "categoria", "valor", "origem"]]
        ...
        st.download_button(
            ...
            file_name=f'relatorio_financeiro_{m_rel.replace("/", "_")}.csv',
            mime='text/csv',
        )
    else:
        st.warning("Sem dados suficientes para gerar o relatório deste mês.")
